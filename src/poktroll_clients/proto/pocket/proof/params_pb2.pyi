"""
@generated by mypy-protobuf.  Do not edit manually!
isort:skip_file
"""

from poktroll_clients.proto.cosmos.base.v1beta1 import coin_pb2 as _coin_pb2
from google.protobuf import descriptor as _descriptor
from google.protobuf import message as _message
import builtins as _builtins
import sys
import typing as _typing

if sys.version_info >= (3, 10):
    from typing import TypeAlias as _TypeAlias
else:
    from typing_extensions import TypeAlias as _TypeAlias

DESCRIPTOR: _descriptor.FileDescriptor

@_typing.final
class Params(_message.Message):
    """Params defines the parameters for the module."""

    DESCRIPTOR: _descriptor.Descriptor

    PROOF_REQUEST_PROBABILITY_FIELD_NUMBER: _builtins.int
    PROOF_REQUIREMENT_THRESHOLD_FIELD_NUMBER: _builtins.int
    PROOF_MISSING_PENALTY_FIELD_NUMBER: _builtins.int
    PROOF_SUBMISSION_FEE_FIELD_NUMBER: _builtins.int
    proof_request_probability: _builtins.float
    """proof_request_probability is the probability of a session requiring a proof
    if it's cost (i.e. compute unit consumption) is below the ProofRequirementThreshold.
    """
    @_builtins.property
    def proof_requirement_threshold(self) -> _coin_pb2.Coin:
        """proof_requirement_threshold is the session cost (i.e. compute unit consumption)
        threshold which asserts that a session MUST have a corresponding proof when its cost
        is equal to or above the threshold. This is in contrast to the this requirement
        being determined probabilistically via ProofRequestProbability.

        TODO_MAINNET_MIGRATION: Consider renaming this to `proof_requirement_threshold_upokt`.
        """

    @_builtins.property
    def proof_missing_penalty(self) -> _coin_pb2.Coin:
        """proof_missing_penalty is the number of tokens (uPOKT) which should be slashed from a supplier
        when a proof is required (either via proof_requirement_threshold or proof_missing_penalty)
        but is not provided.
        TODO_MAINNET_MIGRATION: Consider renaming this to `proof_missing_penalty_upokt`.
        """

    @_builtins.property
    def proof_submission_fee(self) -> _coin_pb2.Coin:
        """proof_submission_fee is the number of tokens (uPOKT) which should be paid by
        the supplier operator when submitting a proof.
        This is needed to account for the cost of storing proofs onchain and prevent
        spamming (i.e. sybil bloat attacks) the network with non-required proofs.
        TODO_MAINNET_MIGRATION: Consider renaming this to `proof_submission_fee_upokt`.
        """

    def __init__(
        self,
        *,
        proof_request_probability: _builtins.float = ...,
        proof_requirement_threshold: _coin_pb2.Coin | None = ...,
        proof_missing_penalty: _coin_pb2.Coin | None = ...,
        proof_submission_fee: _coin_pb2.Coin | None = ...,
    ) -> None: ...
    _HasFieldArgType: _TypeAlias = _typing.Literal["proof_missing_penalty", b"proof_missing_penalty", "proof_requirement_threshold", b"proof_requirement_threshold", "proof_submission_fee", b"proof_submission_fee"]  # noqa: Y015
    def HasField(self, field_name: _HasFieldArgType) -> _builtins.bool: ...
    _ClearFieldArgType: _TypeAlias = _typing.Literal["proof_missing_penalty", b"proof_missing_penalty", "proof_request_probability", b"proof_request_probability", "proof_requirement_threshold", b"proof_requirement_threshold", "proof_submission_fee", b"proof_submission_fee"]  # noqa: Y015
    def ClearField(self, field_name: _ClearFieldArgType) -> None: ...

Global___Params: _TypeAlias = Params  # noqa: Y015
